...
:root {
  ...
  /* shadcn/ui tokens — style: Mira, base color: Neutral */
  ...
  /* Atrium aliases onto the tokens above */
  --surface: var(--card);
  --surface-subtle: var(--muted);
  --muted-light: oklch(0.708 0 0);
  --border-strong: var(--input);
  --accent-soft: var(--accent);
  --private: oklch(0.7 0.16 60);
  --public: oklch(0.6 0.13 155);
  --unlisted: var(--muted-foreground);
  --shadow-sm: none;
}
...
.logo-mark { width: 55%; height: 55%; }
.welcome h1 {
  margin: 2.75rem 0 1.25rem; font-size: clamp(3rem, 9vw, 5.5rem);
  letter-spacing: -.07em; line-height: .92; font-weight: 650;
}
.eyebrow {
  margin: 0 0 .7rem; color: var(--muted-foreground); font-size: .72rem;
  font-weight: 700; letter-spacing: .1em; text-transform: uppercase;
}
.lead { max-width: 33rem; color: var(--muted-foreground); font-size: 1.12rem; line-height: 1.7; }
.button {
  display: inline-flex; min-height: 2.25rem; align-items: center; justify-content: center;
  gap: .5rem; border: 1px solid transparent; border-radius: var(--radius-md);
  padding: 0 1rem; cursor: pointer; font-size: .875rem; font-weight: 500;
  line-height: 1; transition: .15s ease;
}
.button .icon { font-size: 1rem; }
.button-primary { color: var(--primary-foreground); background: var(--primary); box-shadow: var(--shadow-sm); }
.button-primary:hover { opacity: .9; transform: translateY(-1px); }
.button-secondary { border-color: var(--border); background: var(--background); }
.button-secondary:hover { background: var(--accent); color: var(--accent-foreground); }
...
.auth-heading > p:last-child { margin: .6rem 0 1.5rem; color: var(--muted-foreground); font-size: .88rem; line-height: 1.55; }
.form-stack { display: grid; gap: 1rem; }
.form-stack label { display: grid; gap: .45rem; color: var(--foreground); font-size: .8rem; font-weight: 550; }
.form-stack input {
  width: 100%; height: 2.25rem; border: 1px solid var(--border-strong);
  border-radius: var(--radius-md); padding: 0 .75rem; color: var(--foreground);
  background: var(--background); outline: none; box-shadow: var(--shadow-sm);
}
.form-stack input:focus {
  border-color: var(--ring);
  box-shadow: 0 0 0 3px color-mix(in oklch, var(--ring) 50%, transparent);
}
.form-stack small { color: var(--muted-foreground); font-weight: 400; }
.alert {
  margin: 0 0 1.2rem; border: 1px solid var(--border); border-radius: var(--radius-lg);
  padding: .75rem 1rem; color: var(--destructive); font-size: .875rem;
...
.icon-button .icon { font-size: 1rem; }
...
.workspace-avatar, .user-avatar {
  ...
  color: var(--primary-foreground); background: var(--primary); font-size: .72rem; font-weight: 700;
}
.workspace-switcher strong, .workspace-switcher small { display: block; }
.workspace-switcher strong { font-size: .78rem; }
.workspace-switcher small { margin-top: .1rem; color: var(--muted-foreground); font-size: .65rem; text-transform: capitalize; }
...
.nav-item, .page-link {
  ...
  padding: .45rem .55rem; color: var(--muted-foreground); font-size: .78rem;
}
...
.nav-label button { border: 0; color: var(--muted-foreground); background: none; cursor: pointer; }
.inline-create { display: inline-flex; margin: 0; }
.inline-create button {
  border: 0; padding: 0 .2rem; color: var(--muted-foreground); background: transparent;
  cursor: pointer;
}
...
.visibility-dot {
  width: .42rem; height: .42rem; flex-shrink: 0; border-radius: 50%;
...
.user-details span { margin-top: .1rem; color: var(--muted-foreground); font-size: .62rem; }
...
.breadcrumbs { display: flex; gap: .55rem; color: var(--muted-foreground); font-size: .76rem; }
...
.visibility-badge {
  ...
  border-radius: 999px; padding: .35rem .6rem; color: var(--muted-foreground); font-size: .68rem; text-transform: capitalize;
}
.document {
  width: min(100% - 3rem, 90ch); margin: 0 auto; padding: 6rem 0 8rem;
  font-size: .9375rem;
}
.document-meta { display: flex; align-items: center; gap: .55rem; color: var(--muted-foreground); font-size: .72rem; }
.document-icon { display: grid; width: 1.7rem; height: 1.7rem; place-items: center; border-radius: .4rem; background: var(--accent-soft); color: var(--foreground); }
.document h1 { margin: 1.4rem 0 1rem; font-size: 2.75rem; letter-spacing: -.055em; line-height: 1.08; }
.document-lead { color: var(--muted-foreground); font-size: 1.08rem; line-height: 1.7; }
...
.document p { margin: 0 0 1.25em; color: var(--muted-foreground); line-height: 1.75; }
.dashboard h2 { margin: 2.5rem 0 1rem; }
.dashboard-actions { display: flex; flex-wrap: wrap; gap: .65rem; margin-top: 1.75rem; }
.dashboard-action { margin: 0; }
.dashboard-action-button {
  display: inline-flex; align-items: center; gap: .5rem; border: 1px solid var(--border);
  border-radius: var(--radius-md); padding: .6rem 1rem; color: var(--foreground);
  background: var(--surface); font-size: .82rem; font-weight: 550; cursor: pointer;
}
.dashboard-action-button:hover { background: var(--accent); }
.dashboard-action-button:disabled { color: var(--muted-light); cursor: not-allowed; }
.dashboard-recent { display: grid; gap: .6rem; }
.dashboard-recent-item {
  display: grid; grid-template-columns: 1fr auto; align-items: baseline;
  gap: .3rem .75rem; border: 1px solid var(--border); border-radius: var(--radius-lg);
  padding: .9rem 1.1rem; background: var(--surface);
}
.dashboard-recent-item:hover { border-color: var(--border-strong); }
.dashboard-recent-item span {
  grid-column: 1; color: var(--muted-light); font-size: .68rem; text-transform: uppercase;
  letter-spacing: .04em;
}
.dashboard-recent-item strong { grid-column: 1; font-size: .92rem; font-weight: 600; }
.dashboard-recent-item small { grid-column: 2; grid-row: 1 / span 2; color: var(--muted-foreground); font-size: .72rem; }
.callout {
...
.callout p { margin: .25rem 0 0; color: var(--muted-foreground); font-size: .78rem; }
.rendered-markdown h1 { margin-top: 0; }
.rendered-markdown h2, .rendered-markdown h3 { margin-top: 2rem; letter-spacing: -.025em; }
.rendered-markdown p, .rendered-markdown li { margin: 0 0 1.25em; color: var(--muted-foreground); line-height: 1.75; }
.rendered-markdown li:last-child, .rendered-markdown p:last-child { margin-bottom: 0; }
.rendered-markdown ul, .rendered-markdown ol { margin: 0 0 1.25em; padding-left: 1.5em; }
.rendered-markdown li > p { margin-bottom: .5em; }
.rendered-markdown code {
  border: 1px solid var(--border); border-radius: .3rem; padding: .1rem .3rem;
  background: var(--surface-subtle); font-family: var(--font-mono); font-size: .86em;
}
...
.visibility-control {
  display: grid; gap: .15rem; color: var(--muted-foreground); font-size: .58rem;
...
.format-toolbar button {
  ...
  border-radius: var(--radius-sm); color: var(--muted-foreground); background: transparent;
...
.preview-label {
  ...
  color: var(--muted-foreground); background: var(--background); font-size: .66rem; text-transform: uppercase;
}
...
.public-header {
  ...
  color: var(--muted-foreground); background: var(--surface); font-size: .76rem;
}
...
.upload-bar label { display: flex; align-items: center; gap: .6rem; color: var(--muted-foreground); font-size: .7rem; }
.settings-topbar {
  ...
  color: var(--muted-foreground); background: var(--surface); font-size: .72rem;
}
...
.settings-heading p:not(.eyebrow), .standalone-card p { color: var(--muted-foreground); line-height: 1.6; }
...
.search-form input, .member-row select {
  min-height: 2.25rem; border: 1px solid var(--border-strong); border-radius: var(--radius-md);
...
.success-card p { color: var(--muted-foreground); font-size: .8rem; }
...
.member-copy span, .revision-row span { margin-top: .15rem; color: var(--muted-foreground); font-size: .68rem; }
.member-row form { display: flex; gap: .4rem; }
.button-danger {
  border-color: color-mix(in oklch, var(--destructive) 30%, transparent);
  color: var(--destructive); background: transparent;
}
.you-label { width: 4rem; text-align: center; color: var(--muted-foreground); font-size: .68rem; }
...
.search-result > span { color: var(--muted-foreground); font-size: .65rem; }
.search-result > strong { margin-top: .25rem; font-size: .9rem; }
.search-result p { margin: .45rem 0 0; color: var(--muted-foreground); font-size: .76rem; line-height: 1.5; }
...
.form-link { justify-self: center; color: var(--muted-foreground); font-size: .75rem; }
...
.qr-code {
  display: inline-block; margin: .25rem 0 .9rem; border: 1px solid var(--border);
  border-radius: var(--radius-lg); padding: .9rem; background: #fff;
}
.qr-code svg { display: block; width: 9rem; height: 9rem; }
.uri-code { margin-top: .5rem; font-size: .68rem; }
.mfa-form { margin-top: 1rem; grid-template-columns: 1fr auto; }
.auth-divider { display: flex; align-items: center; gap: .7rem; margin: 1rem 0; color: var(--muted-foreground); font-size: .7rem; }
...
.empty-state {
  display: grid; justify-items: center; gap: .5rem; padding: 3rem 1rem;
  color: var(--muted-foreground); text-align: center;
}
.empty-state .icon { width: 1.75rem; height: 1.75rem; color: var(--muted-light); }
.empty-state strong { color: var(--foreground); font-size: .85rem; font-weight: 600; }
.empty-state span { max-width: 26rem; color: var(--muted-foreground); font-size: .75rem; }
...
.book-stack {
  ...
  background:
    linear-gradient(var(--background) 30%, transparent) 0 0 local,
    linear-gradient(transparent, var(--background) 70%) 0 100% local,
    linear-gradient(color-mix(in oklch, var(--foreground) 16%, transparent), transparent 30%) 0 0 scroll,
    linear-gradient(transparent, color-mix(in oklch, var(--foreground) 16%, transparent) 70%) 0 100% scroll;
...
.rail-account-menu {
  ...
  display: grid; width: 14rem; border: 1px solid var(--border);
  border-radius: var(--radius-md); padding: .375rem; background: var(--surface);
  box-shadow: 0 14px 40px rgba(0,0,0,.14);
}
.rail-account-menu strong, .rail-account-menu > span {
  overflow: hidden; padding: .3rem .5rem 0; text-overflow: ellipsis; white-space: nowrap;
}
.rail-account-menu strong { font-size: .875rem; font-weight: 600; }
.rail-account-menu > span {
  margin-bottom: .3rem; padding-top: .05rem; color: var(--muted-foreground); font-size: .75rem;
}
.rail-account-menu > span::after {
  display: block; margin-top: .55rem; border-top: 1px solid var(--border); content: "";
}
.rail-account-menu a, .rail-account-menu button {
  display: flex; align-items: center; gap: .55rem; width: 100%; min-height: 2.15rem;
  border: 0; border-radius: var(--radius-sm);
  padding: 0 .5rem; text-align: left; color: var(--foreground);
...
.document-panel > header > form > button {
  display: grid; width: 1.85rem; height: 1.85rem; place-items: center;
  border: 0; border-radius: var(--radius-sm); color: var(--muted-foreground);
...
.document-panel .book-title-form button {
  width: 0; height: 1.85rem; flex: 0 0 auto; opacity: 0; overflow: hidden;
  pointer-events: none; transition: opacity .12s ease, width .12s ease;
}
.document-panel .book-title-form button.book-title-save-visible {
  width: 1.85rem; opacity: 1; pointer-events: auto;
}
.document-panel nav {
...
.document-panel nav a {
  ...
  color: var(--muted-foreground); font-size: .72rem; cursor: grab;
}
.document-panel nav a[dragging] { opacity: .4; }
...
.global-search {
  ...
  color: var(--muted-foreground); background: var(--background); font-size: .68rem;
}
...
.search-dialog-actions { display: grid; gap: .1rem; padding-bottom: .3rem; }
.search-dialog-action {
  display: flex; width: 100%; align-items: center; border: 0; border-radius: .5rem;
  padding: .62rem .7rem; color: var(--foreground); background: transparent;
  cursor: pointer; text-align: left; font-size: .78rem; font-weight: 550;
}
.search-dialog-results .search-dialog-result-active { background: var(--accent-soft); }
.dialog {
  width: min(calc(100% - 2rem), 26rem); border: 1px solid var(--border);
  border-radius: var(--radius-lg); padding: 1.5rem; color: var(--foreground);
  background: var(--background); box-shadow: 0 24px 80px rgba(0,0,0,.28);
}
.dialog::backdrop { background: rgba(0,0,0,.46); backdrop-filter: blur(2px); }
...
.dialog-header h2 { margin: 0; font-size: 1.05rem; font-weight: 650; letter-spacing: -.02em; }
.dialog-description { margin: -.5rem 0 0; color: var(--muted-foreground); font-size: .82rem; line-height: 1.5; }
.dialog-close {
  display: grid; flex: 0 0 auto; width: 1.6rem; height: 1.6rem; place-items: center;
  border: 0; border-radius: var(--radius-sm); color: var(--muted-foreground);
  background: transparent; cursor: pointer;
}
.dialog-close .icon { font-size: .85rem; }
.dialog-close:hover { color: var(--foreground); background: var(--surface-subtle); }
.dialog-footer {
  display: flex; justify-content: flex-end; gap: .6rem; margin-top: .25rem;
  border-top: 1px solid var(--border); padding-top: 1rem;
}
.settings-dialog {
  width: min(calc(100% - 2rem), 58rem); max-height: min(40rem, calc(100vh - 4rem));
  overflow-y: auto; padding: 0;
}
.settings-dialog .dialog-shell { position: relative; display: block; gap: 0; }
.settings-dialog .dialog-close { position: absolute; top: 1.1rem; right: 1.1rem; z-index: 1; }
.settings-dialog .dialog-body { padding: 1rem 1.5rem 1.5rem; }
.settings-dialog .settings-page { width: auto; padding: 1.5rem 0 0; }
.settings-dialog .settings-topbar { display: none; }
.toast-stack {
  position: fixed; z-index: 80; right: 1rem; bottom: 1rem; display: grid; gap: .5rem;
  width: min(calc(100% - 2rem), 22rem);
}
.toast {
  display: flex; align-items: flex-start; gap: .6rem; border: 1px solid var(--border);
  border-radius: var(--radius-lg); padding: .75rem .9rem; color: var(--foreground);
  background: var(--surface); box-shadow: 0 14px 40px rgba(0,0,0,.16); font-size: .8rem;
  animation: toast-in .15s ease;
}
.toast-error { border-color: color-mix(in oklch, var(--destructive) 40%, transparent); }
.toast-dot {
  flex: 0 0 auto; width: .5rem; height: .5rem; margin-top: .3rem; border-radius: 50%;
...
.tabs {
  display: flex; gap: 1.25rem; border-bottom: 1px solid var(--border); margin-bottom: 1.5rem;
}
.tab {
  display: inline-flex; align-items: center; height: 2.75rem; border-bottom: 2px solid transparent;
  color: var(--muted-foreground); font-size: .8rem; font-weight: 550;
}
.tab:hover { color: var(--foreground); }
.tab-active { color: var(--foreground); border-bottom-color: var(--foreground); }
.document .tabs + h1 { margin-top: 0; }
...
.search-dialog header {
  ...
  padding: 0 .85rem; color: var(--muted-foreground);
}
.search-dialog input {
...
.search-dialog-heading {
  padding: .5rem .6rem .3rem; color: var(--muted-light); font-size: .62rem;
  font-weight: 650; letter-spacing: .06em; text-transform: uppercase;
}
.search-dialog-results > p { padding: 1.5rem; color: var(--muted-foreground); text-align: center; font-size: .75rem; }
...
.search-dialog-results a small {
  overflow: hidden; margin-top: .2rem; color: var(--muted-foreground); font-size: .68rem;
...
.compact-button { min-height: 2rem; padding: 0 .75rem; }
...
.topbar .button { min-height: 2rem; padding: 0 .75rem; }
...
.quiet-action {
  ...
  color: var(--muted-foreground); font-size: .75rem; font-weight: 550;
}
...
.visibility-menu summary {
  ...
  border-radius: var(--radius-sm); padding: 0 .55rem; color: var(--muted-foreground);
...
.visibility-popover, .page-menu-popover {
  ...
  width: 13rem; border: 1px solid var(--border); border-radius: var(--radius-md);
  padding: .25rem; background: var(--surface);
  box-shadow: 0 12px 30px rgba(0,0,0,.12);
}
.visibility-popover label {
  display: flex; align-items: center; justify-content: space-between;
  gap: .75rem; padding: .4rem; color: var(--muted-foreground); font-size: .7rem;
}
...
.page-menu-popover a, .page-menu-popover button {
  ...
  text-align: left; color: var(--muted-foreground); background: transparent;
  cursor: pointer; font-size: .72rem; font-weight: 550;
}
.page-menu-popover a:hover, .page-menu-popover button:hover {
  color: var(--foreground); background: var(--surface-subtle);
}
.page-menu-popover .danger-action { color: var(--destructive); }
.page-menu-popover .danger-action:hover {
...
.book-icon-field {
  ...
  gap: .55rem; color: var(--muted-foreground); font-size: .68rem;
}
...
.visual-editor { background: var(--surface); }
.visual-editor-canvas {
  height: calc(100vh - 3.25rem); overflow-y: auto; background: var(--surface);
...
.visual-editor .title-input {
  width: 100%; height: auto; border: 0; padding: 0; margin-bottom: 1.75rem;
  font-size: 2.6rem; line-height: 1.1; letter-spacing: -.055em;
}
.floating-format-toolbar {
  position: fixed; z-index: 10; bottom: 1.75rem; left: 50%;
  transform: translateX(-50%); width: max-content;
  max-width: calc(100% - 2rem); min-height: 2.45rem; margin: 0;
  border: 1px solid var(--border); border-radius: .55rem; padding: .25rem;
  background: color-mix(in srgb, var(--surface) 97%, transparent);
  box-shadow: 0 10px 30px rgba(0,0,0,.16); backdrop-filter: blur(12px);
}
.floating-format-toolbar select {
  height: 1.8rem; border: 0; padding: 0 .45rem; color: var(--muted-foreground);
  background: transparent; outline: none; font-size: .72rem;
}
.visual-content {
  min-height: 25rem; outline: none; font-size: .9375rem; line-height: 1.75;
}
.visual-content p, .visual-content ul, .visual-content ol { margin: 0 0 1.25em; }
.visual-content ul, .visual-content ol { padding-left: 1.5em; }
...
.visual-content blockquote {
  ...
  padding-left: 1rem; color: var(--muted-foreground);
}
.visual-content img {
  display: block; max-width: 100%; height: auto; border-radius: var(--radius-lg);
}
...
.slash-menu {
  position: fixed; z-index: 40; display: grid; width: 12rem;
...
.slash-menu button:hover, .slash-menu button.slash-menu-active {
  color: var(--foreground); background: var(--surface-subtle);
}
...
@media (max-width: 760px) {
  ...
  .visual-editor-canvas { padding: 2.5rem 1.25rem 6rem; }
  .editor-page { min-height: calc(100vh - 3.25rem); }
  .visual-editor .title-input { font-size: 2rem; }
  .visibility-menu { display: none; }
  .floating-format-toolbar { bottom: calc(3.75rem + 1rem); }
}
`;
